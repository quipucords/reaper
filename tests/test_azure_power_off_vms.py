...
def create_mock_vm(
    ...
    """Create a Mock object that looks like an Azure Virtual Machine."""
    mock_unrelated_status = Mock()
    mock_unrelated_status.code = "unrelated/status"
    mock_running_status = Mock()
    mock_running_status.code = "PowerState/running" if is_running else "PowerState/off"
    mock_another_status = Mock()
    mock_another_status.code = "another/status"

    mock_vm = Mock()
    mock_vm.name = name if name else str(uuid.uuid4())
    mock_vm.id = (
        resource_id
        if resource_id
        else synthesize_resource_id(resource_group=resource_group)
    )
    if include_statuses:
    ...
    mock_vm.tags = tags if tags else None
    return mock_vm
...
def test_vm_is_running():
    ...
    mock_vm = create_mock_vm(is_running=True, include_statuses=True)
    assert reaper.azure_power_off_vms.vm_is_running(mock_vm) is True
    mock_vm = create_mock_vm(is_running=False, include_statuses=True)
...
def test_has_bypass_tag():
    ...
    mock_vm = create_mock_vm(tags=tags_with_bypass, include_statuses=False)
    ...
    mock_vm = create_mock_vm(tags=tags_without_bypass, include_statuses=False)
    assert reaper.azure_power_off_vms.has_bypass_tag(mock_vm) is False

    mock_vm = create_mock_vm(tags={}, include_statuses=False)
    assert reaper.azure_power_off_vms.has_bypass_tag(mock_vm) is False

    mock_vm = create_mock_vm(tags=None, include_statuses=False)
    assert reaper.azure_power_off_vms.has_bypass_tag(mock_vm) is False
...
def test_get_resource_group_name():
    ...
    mock_vm = create_mock_vm(resource_group=resource_group)
...
def test_power_off_vm():
    ...
    name = str(uuid.uuid4())
    resource_group = str(uuid.uuid4())
    mock_vm = create_mock_vm(name=name, resource_group=resource_group)
    mock_client = Mock()
...
def test_handle_regular_vms():
    ...
    mock_vm_running_with_bypass = create_mock_vm(
